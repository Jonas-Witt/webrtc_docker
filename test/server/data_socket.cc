...
std::string DataSocket::request_arguments() const {
  ...
  if (args != std::string::npos) return request_path_.substr(args + 1);
  return "";
}
...
bool DataSocket::ParseHeaders() {
  ...
  if (!ParseMethodAndPath(request_headers_.data(), i)) return false;
  ...
  if (method_ == POST) {
    ...
    if (!ParseContentLengthAndType(headers, len)) return false;
  }
...
bool DataSocket::ParseMethodAndPath(const char* begin, size_t len) {
  ...
  if (!path || path >= end) return false;

  ++path;
  begin = path;
  while (!isspace(*path) && path < end) ++path;
