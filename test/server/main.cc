...
int main(int argc, char* argv[]) {
  ...
  ros::NodeHandle nodehandle("test");
  ros::Publisher chatter_pub =
      nodehandle.advertise<std_msgs::String>("chatter", 1000);
  ...
  std::string usage = "Example usage: " + program_name + " 8888";
  int port = 8888;
  ...
  while (!quit) {
    fd_set socket_set;
    FD_ZERO(&socket_set);
    if (listener.valid()) FD_SET(listener.socket(), &socket_set);
