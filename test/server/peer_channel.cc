...
ChannelMember* PeerChannel::Lookup(DataSocket* ds) const {
  ...
  for (; i < ARRAYSIZE(kRequestPaths); ++i) {
    if (ds->PathEquals(kRequestPaths[i])) break;
  }

  if (i == ARRAYSIZE(kRequestPaths)) return NULL;
  ...
  for (; iter != members_.end(); ++iter) {
    if (id == (*iter)->id()) {
      if (i == kWait) (*iter)->SetWaitingSocket(ds);
      if (i == kSignOut) (*iter)->set_disconnected();
      return *iter;
    }
  }
...
ChannelMember* PeerChannel::IsTargetedRequest(const DataSocket* ds) const {
  ...
  if (args == std::string::npos) return NULL;
  size_t found;
